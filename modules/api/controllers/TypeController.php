...
class TypeController extends Controller
{
    ...
    {
        $where = [];
        $selectColumns = Util::filterQuerySelectColumns(['t.id', 't.name', 't.ordering', 't.enabled', 't.created_at', 't.created_by', 't.updated_at', 't.updated_by'], $fields);
        // Order By
        $orderByColumns = [];
        if (!empty($orderBy)) {
        ...
        $query = (new ActiveQuery(Type::className()))
            ->select($selectColumns)
            ->from('{{%type}} t')
            ->where($where)
            ->offset($offset)
            ->limit($limit)
            ->orderBy($orderByColumns ?: ['t.ordering' => SORT_ASC]);
    ...
    {
        $model = (new ActiveQuery(Type::className()))
            ->from('{{%type}}')
            ->where(['id' => (int) $id])
            ->one();
